...
st.markdown("""
Con estos 3 mini-proyectos demuestro mis skills en Python para el rol de intern en portafolios líquidos:
    ...
    if st.button("▶️ Ejecutar macro_flag.py"):
        ...
        if files:
            with open(files[-1],"rb") as fp:
                st.download_button("⬇️ Descargar Excel", fp, file_name=files[-1])
...
with tab2:
    ...
    if csv:
        ...
        with open(os.path.join("reports",rpt),"rb") as fp:
            st.download_button("⬇️ Descargar Reporte", fp, file_name=rpt)
...
with tab3:
    ...
    """)
    th = st.slider("Umbral", -1.0, 1.0, -0.2, 0.05)
